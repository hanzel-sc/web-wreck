...
export function outputMermaid(graph: ExecutionGraph, analysis: AuthAnalysis): string {
  ...
  for (const route of graph.routes) {
    ...
    const highSeverity = findings.some(f => f.severity === 'critical' || f.severity === 'high');

    let style = ':::safe';
    if (isVulnerable) style = ':::vulnerable';
    else if (highSeverity) style = ':::warning';

    const countLabel = findings.length > 0 ? ` (${findings.length} issues)` : '';
    const label = `${route.method} ${route.path}${countLabel}`;
  ...
  lines.push('  classDef warning fill:#ff922b,stroke:#d9480f,stroke-width:2px');
