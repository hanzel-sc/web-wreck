...
import { analyzeAuthPresence } from './analyze/index.js';
...
async function main() {
  ...
  try {
    ...
    const jsonOutput = outputJson(graph, analysis);
    const mermaidOutput = outputMermaid(graph, analysis);
    const htmlOutput = outputHtml(graph, analysis);
